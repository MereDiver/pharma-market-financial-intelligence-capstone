...
Default analysis is intentionally bounded to California, Texas, New York, Florida, and Illinois for 2024–2025. The verified five-state run with 20 openFDA products completed successfully in approximately 45 minutes on Databricks Free Edition; runtime is environment-dependent. `MEDICAID_STATES` can expand later. The top 20 products by reimbursement are submitted for openFDA enrichment by default, and `MAX_OPENFDA_PRODUCTS` remains configurable up to 100. No synthetic reimbursement records are generated or required.
